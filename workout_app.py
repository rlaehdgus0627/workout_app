...
def save_routines(routines):
    with open(ROUTINES_FILE, "w", encoding="utf-8") as f:
        json.dump([routine.to_dict() for routine in routines], f, indent=4, ensure_ascii=False)
...
class CreateRoutineWindow(tk.Toplevel):
    ...
    def save_routine(self):
        ...
        for r in routines:
            if r.name == name:
                messagebox.showerror("오류", "같은 이름의 루틴이 이미 존재합니다.")
                return
        if not self.exercises:
            messagebox.showerror("오류", "운동을 최소 한 개 이상 추가하세요.")
            return
        routine = Routine(name, self.exercises)
...
class WorkoutSessionWindow(tk.Toplevel):
    ...
    def build_widgets(self):
        ...
        self.previous_record_label = ttk.Label(self.content_frame, text="", font=("Arial", 10), foreground="blue")
        self.previous_record_label.pack(pady=2)
    ...
    def record_current_exercise(self):
        if self.current_exercise_sets:
            ...
            exercise_log = {
                ...
                "total_volume": sum(s.volume() for s in self.current_exercise_sets)
            }
    ...
    def compare_with_previous(self):
        ...
        for l in reversed(logs):
            if l["routine_name"] == routine_name:
                previous_log = l
                break
        info = ""
        if previous_log:
            ...
            used_muscles = {ex.muscle_group for ex in self.routine.exercises if ex.muscle_group}
            for mg in used_muscles:
    ...
    def show_graph(self):
        ...
        for log in logs:
            ...
            if date not in data_by_date:
                data_by_date[date] = 0
            data_by_date[date] += log["muscle_volumes"].get(selected_mg, 0)
        dates = sorted(data_by_date.keys())
        date_objs = [datetime.datetime.strptime(d, "%Y-%m-%d") for d in dates]
        volumes = [data_by_date[d] for d in dates]

        fig, ax = plt.subplots(figsize=(8, 4))
        ax.plot(date_objs, volumes, marker="o", label=selected_mg)
        ax.set_xlabel("날짜")
        ax.set_ylabel("볼륨")
        ax.set_title(f"날짜별 {selected_mg} 볼륨")
        ax.legend()
        fig.autofmt_xdate()
